from layers.DCRNN_related import *
import torch
...
class DCRNN(nn.Module):
    ...
    def calculate_rate(self,batch_idx,epoch):
        '''在训练过程中不断减小，直到为0-->刚开始模型没有训练好的时候，
        就用label进行Decoder的训练，防止由于误差累计导致的训练偏差'''
        ...
        if global_step >=10*k:
            ...
            teacher_forcing_ratio=k / (k + math.exp(global_step / k))
        return teacher_forcing_ratio


    def forward(self, input,adj,**kwargs):
        source=input.transpose(1,-1) #(B,L,N,C)
        target=kwargs.get('targets').transpose(1,-1) #(B,L,N,C)
        batch_idx=kwargs.get('index')
        epoch=kwargs.get('epoch')
        if self.training:# 如果在训练 那么可以使用teacher forcing mechanism
            teacher_forcing_ratio = self.calculate_rate(batch_idx,epoch=epoch)
        else:# 如果否则就不使用
            teacher_forcing_ratio=0
        ...
        target = torch.cat([self.GO_Symbol, target], dim=0) # 全零和对应的target进行拼接，在长度维度上进行Concat
        ...
        outputs = self.decoder(target, context, teacher_forcing_ratio=teacher_forcing_ratio)#outputs第一个时间步的时全零值
        # the elements of the first time step of the outputs are all zeros.
        outputs=outputs[1:, :, :]  # (seq_length, batch_size, num_nodes*output_dim)  (12, 64, 207*1)
        outputs=outputs.reshape(self._batch_size,self._output_dim,self._num_nodes,-1)
        return outputs #(B,C,N,L)
...
class DCRNNEncoder(nn.Module):
    ...
    def forward(self, inputs, initial_hidden_state):
        ...
        inputs = torch.reshape(inputs, (seq_length, batch_size, -1))  # (L, B, 207*2) #将节点个数和特征个数进行融合
...
class DCGRUDecoder(nn.Module):
    ...
    def forward(self, inputs, initial_hidden_state, teacher_forcing_ratio=0.5):
        ...
        outputs = torch.zeros(seq_length, batch_size, self._num_nodes*self._output_dim,device='cuda')  # (13, 50, 207*1)
        ...
        current_input = inputs[0]  # the first input to the rnn is GO Symbol(主要是为了进行初始化)
        for t in range(1, seq_length):
            # hidden_state = initial_hidden_state[i_layer]  # i_layer=0, 1, ...
            next_input_hidden_state = []
            for i_layer in range(0, self._num_rnn_layers):
                hidden_state = initial_hidden_state[i_layer]
                output, hidden_state = self.decoding_cells[i_layer](current_input, hidden_state)# 其实可以不用这样直接将Encoder的state作为Decoder的state就行
