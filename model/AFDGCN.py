...
class AFDGCN(nn.Module):
    def __init__(self, num_node, input_dim, hidden_dim, output_dim, embed_dim, cheb_k, horizon, num_layers, heads,
        ...
        self.conv_pred=nn.Conv2d(self.timesteps,self.horizon,kernel_size=(1,1),bias=True)

    def forward(self, x,adj,**kwargs):
        x=x.transpose(1,-1)  # x: (B, T, N, D)
        batch_size = x.shape[0]
        x = self.feature_attention(x)  # 论文中的特征增强模块，但其实就是两个门控机制
        init_state = self.encoder.init_hidden(batch_size)
        output, _ = self.encoder(x, init_state, self.node_embedding)  # (B, T, N, hidden_dim) 这个就是AGCRN中的GCN
        state = output[:, -1:, :, :]  # 得到最后一个时间步的hidden状态
        state = self.nconv(state)
        SAtt = self.GraphAttentionLayer(state)
        TAtt = self.MultiHeadAttention(output).permute(0, 2, 1, 3)
        out = SAtt + self.conv_pred(TAtt)
        out = self.end_conv(out.permute(0, 3, 2, 1))  # [B, 1, N, T] -> [B, N, T]
        out = out.permute(0, 3, 2, 1)  # [B, T, N,1]
...
class AVWGCN(nn.Module):
    def __init__(self, in_dim, out_dim, cheb_k, embed_dim):
        ...
        self.cheb_k = cheb_k
        self.weights_pool = nn.Parameter(torch.randn((embed_dim, cheb_k, in_dim, out_dim)))
        self.bias_pool = nn.Parameter(torch.randn((embed_dim, out_dim)))
    ...
    def forward(self, x, node_embedding):
        ...
        # (N, D) * (D, C_out) -> (N, C_out)
        bias = torch.einsum('nd, do->no', node_embedding, self.bias_pool)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 考虑上下文的长期依赖信息
        scores = torch.matmul(Q, K.transpose(-1, -2)) / torch.sqrt(torch.tensor(d_k, dtype=torch.float))  # 时间Attention
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, h):
        ...
        attention = torch.where(self.adj > 0, e, zero_vec)  # 这一步就相当于是只要邻居节点
