...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, x):
        x_in = self.align(x)[:, :, self.Kt - 1:, :] # 开头不要
...
class ChebGraphConv(nn.Module):
    def __init__(self, c_in, c_out, Ks, adj, bias):
    ...
    def forward(self, x):
        ...
        if self.Ks - 1 < 0:
            ...
            x_0 = x
            x_1 = torch.einsum('hi,btij->bthj', self.adj, x)
            x_list = [x_0, x_1]
        elif self.Ks - 1 >= 2:
            x_0 = x
            x_1 = torch.einsum('hi,btij->bthj', self.adj, x)
            ...
            for k in range(2, self.Ks):
                x_list.append(torch.einsum('hi,btij->bthj', 2 * self.adj, x_list[k - 1]) - x_list[k - 2])
...
class GraphConv(nn.Module):
    def __init__(self, c_in, c_out, adj, bias):
...
class GraphConvLayer(nn.Module):
    def __init__(self, graph_conv_type, c_in, c_out, Ks, adj, bias):
        ...
        if self.graph_conv_type == 'cheb_graph_conv':
            self.cheb_graph_conv = ChebGraphConv(c_out, c_out, Ks, adj, bias)
        elif self.graph_conv_type == 'graph_conv':
            self.graph_conv = GraphConv(c_out, c_out, adj, bias)
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, adj, bias, droprate):
        super(STConvBlock, self).__init__()
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], n_vertex, act_func)
        self.graph_conv = GraphConvLayer(graph_conv_type, channels[0], channels[1], Ks, adj, bias)
    ...
    def forward(self, x):
        x = self.tmp_conv1(x)# 时间卷积
        x = self.graph_conv(x) # GCN
        x = self.relu(x)
        x = self.tmp_conv2(x)
        x = self.tc2_ln(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) # 层归一化
        x = self.dropout(x)
